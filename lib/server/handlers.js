...
import { uploadToS3, deleteFromS3, getPresignedUrl } from './s3.js';
import {
  MAX_PDF_FILE_BYTES,
...
function parseDataUrl(dataUrl) {
  ...
  const m = /^data:([a-zA-Z0-9]+\/[a-zA-Z0-9.+-]+);base64,(.+)$/i.exec(dataUrl.trim());
  if (!m) return null;
...
export async function handleMySites(req, res) {
  ...
  try {
    ...
    const rows = await sql`
      SELECT id, slug, data, updated_at, view_count
      FROM sites
    ...
    const convRows = await sql`
      SELECT id, type, content, result_url, created_at
      FROM converts
      WHERE user_id = ${userId}
    ...
    return res.status(200).json({ sites, converts: convRows });
  } catch (error) {
    console.error('MySites error:', error);
    return res.status(500).json({ error: 'Xatolar yuz berdi: ' + error.message });
  }
}

export async function handleSaveConvert(req, res) {
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }
  ...
  try {
    const { type, content, resultUrl } = req.body || {};
    if (!type || !content || !resultUrl) {
      return res.status(400).json({ error: 'Ma’lumotlar yetarli emas' });
    }

    await initDb();
    await sql`
      INSERT INTO converts (user_id, type, content, result_url)
      VALUES (${userId}, ${type}, ${content}, ${resultUrl})
    `;

    return res.status(200).json({ success: true });
  } catch (error) {
    console.error('SaveConvert error:', error);
    return res.status(500).json({ error: 'Saqlashda xato' });
  }
}
...
export async function handleSiteDelete(req, res) {
  ...
  try {
    ...
    const result = await sql`
      DELETE FROM sites WHERE slug = ${slug}
      RETURNING slug
    `;
...
export async function handleUploadBlob(req, res) {
  ...
  try {
    ...
    const maxBytes = kind === 'background' ? 6 * 1024 * 1024 : kind === 'convert' ? 5 * 1024 * 1024 : 2 * 1024 * 1024;
    ...
    if (parsed.mime === 'application/pdf') {
      ...
      ext = parsed.mime.split('/')[1] || 'jpg';
      if (ext === 'jpeg') ext = 'jpg';
      if (ext === 'svg+xml') ext = 'svg';
      if (ext === 'x-icon') ext = 'ico';
    }

    const folder = kind === 'background' ? 'backgrounds' : kind === 'convert' ? 'converts' : 'avatars';
    const pathname = `${folder}/${randomUUID()}.${ext}`;

    const s3Result = await uploadToS3(pathname, parsed.buffer, parsed.mime);

    return res.status(200).json({ url: s3Result.url });
  } catch (error) {
...
export async function handleUploadPdf(req, res) {
  ...
  if (!process.env.AWS_S3_BUCKET) return res.status(503).json({ error: 'S3 not configured' });
  ...
  try {
    ...
    if (!filename || contentType !== 'application/pdf') {
      return res.status(400).json({ error: 'Faqat PDF fayllar ruxsat etiladi' });
    }
    ...
    await initDb();
    const used = await getTotalPdfBytesForUser(userId);
    if (used + size > MAX_USER_TOTAL_PDF_BYTES) {
      return res.status(400).json({ error: 'PDF limiti to‘ldi' });
    }

    const key = `pdfs/${userId}/${randomUUID()}-${filename}`;
    const { url, uploadUrl } = await getPresignedUrl(key, contentType);

    return res.status(200).json({ url, uploadUrl });
  } catch (err) {
    console.error('upload-pdf error:', err);
    return res.status(500).json({ error: 'S3 xatosi' });
  }
}
  ...
  try {
    ...
    if (blobUrl) {
      try {
        ...
        } else if (blobUrl.includes('amazonaws.com')) {
          await deleteFromS3(blobUrl);
        }
      } catch (err) {
        console.error('remove-download-item file del:', err);
      }
    }
...
const ROUTES = {
  ...
  'save-convert': handleSaveConvert,
